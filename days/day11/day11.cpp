...
struct monkey_s {
    ...
    void add(uint64_t w) { items.push_back(w); }
};

void turn(monkey_s& m, int part);

struct barrel_s {
    vector<monkey_s> ms;
    vector<monkey_s> save_ms;

    void add(monkey_s& m) { ms.push_back(m); }

    void add_to(uint32_t to, uint64_t w) { ms[to].add(w); }
