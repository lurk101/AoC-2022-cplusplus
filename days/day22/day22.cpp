...
const int board_height = 202;
const int board_width = 152;

vector<vector<state>> board(board_height, vector<state>(board_width, vacuum));
...
int part1() {
    ...
    while (instr_ix != instructions.length()) {
        ...
        while (instr_ix < instructions.length() && isdigit(instructions[next_ix]))
            next_ix++;
        ...
        for (int y = 1; y < board[x].size(); y += side_len)
            if (board[x][y] != vacuum) {
                for (int xx = x; xx < x + side_len; ++xx)
                    for (int yy = y; yy < y + side_len; ++yy)
                        sides[{int(xx), int(yy)}] = side_ix;
                ++side_ix;
            }
    int instr_ix = 0;
