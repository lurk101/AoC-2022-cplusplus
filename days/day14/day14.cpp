...
point get_point(stringstream& ss) {
    char c;
    point p;
    ss >> p.x;
    ss >> c;
...
void add_seam(string s) {
    ...
    while (delim == "->") {
        ...
        if (start.x == next.x) {
            if (start.y < next.y)
                for (int y = start.y; y <= next.y; y++)
                    cave[start.x].insert(y);
            else
                for (int y = start.y; y >= next.y; y--)
                    cave[start.x].insert(y);
        } else {
            if (start.x < next.x)
                for (int x = start.x; x <= next.x; x++)
                    cave[x].insert(start.y);
            else
                for (int x = start.x; x >= next.x; x--)
                    cave[x].insert(start.y);
        }
