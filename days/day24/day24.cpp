...
const point directions[5] = {{1, 0}, {0, 1}, {-1, 0}, {0, -1}, {0, 0}};

bool in_range(int min, int val, int max) { return min <= val && val <= max; }
...
vector<point> blizzards, up_blizzards, down_blizzards, left_blizzards, right_blizzards;
int max_x, max_y;
point start, target;
box bounds({0, 0, 0, 0});

int traverse(point a, point b) {
    int t = 0;
