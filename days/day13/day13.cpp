...
fragment parse(stringstream& ss) {
    auto f = new fragment;
    char c = ss.peek();
    if (c == '[') {
        ss >> c;
...
result compare_fragment(const fragment& m1, const fragment& m2);

result compare_fragment_list(const fragment& m1, const fragment& m2) {
    ...
    if (im1 == m1.numbers.end() && im2 == m2.numbers.end())
        return unknown;
...
bool compare(const fragment& m1, const fragment& m2) { return compare_fragment(m1, m2) == good; }
...
int main() {
    ...
    while (getline(f, s)) {
        ...
        msgs.push_back(msg2);
        getline(f, s);
        if (compare_fragment(msg1, msg2) == good)
            sum += index;
    }
    ss.str("[[2]]");
    msg1 = parse(ss);
