...
struct sensor_s {
    int64_t x, y, dist;
    sensor_s(int64_t sx, int64_t sy, int64_t dst) : x(sx), y(sy), dist(dst){};
};

struct intvl_s {
    int64_t l, r;
    bool operator<(const intvl_s& intvl) const {
        if (l == intvl.l)
            return r < intvl.r;
        return l < intvl.l;
    }
...
int part1(vector<string> lines) {
    ...
    return count + cur_intvl.r - cur_intvl.l;
}
    ...
    for (int64_t cur_y = 0; cur_y <= MAX_VAL; cur_y++) {
        ...
        for (intvl_s intvl : intvls) {
            if (cur_intvl.l <= intvl.l && intvl.r <= cur_intvl.r)
                ...
                return cur_y + MAX_VAL * (cur_intvl.r + 1);
        }
    }
    return 0;
}
